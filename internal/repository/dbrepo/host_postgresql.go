...
func (m *postgresDBRepo) GetHostByID(id int) (models.Host, error) {
	...
	query := `
	SELECT
		id, host_name, canonical_name, url, ip, ipv6,
...
func (m *postgresDBRepo) GetAllServiceStatusCounts() (models.AllServiceStatusCounts, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	var res models.AllServiceStatusCounts
